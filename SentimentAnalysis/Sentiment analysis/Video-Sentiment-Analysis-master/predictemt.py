...
"""Predictemt.py for predicting frames in video based on the trained model"""
...
label_to_text = {0: 'Angry birds :|', 1: 'Disgusted folks', 2: 'Anxious peeps', 3: 'Delighted ones :)',
                 4: 'Unhappy ones :('}


def pred(img_path):
    label_to_text = {0: 'Hmm,Angry birds :|', 1: 'Disgusted folks', 2: 'Anxious peeps', 3: 'Delighted ones :)',
                     4: 'Unhappy ones :('}
    img = cv2.imread(img_path)  # read Image
    gray_fr = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)  # covert image to grayscale
    faces_rects = facec.detectMultiScale(gray_fr, scaleFactor=1.2,
                                         minNeighbors=5)  # opencv's cascade classifier will be used for detecting the face
    if len(faces_rects) != 0:
        ...
        return label_to_text[
                   np.argmax(loaded_model.predict(img))], img  # model.predict is used for predicting the emotion
    else:
...
def ssimscore1(im1, im2):
    ...
    (score, diff) = structural_similarity(im1, im2, full=True,
                                          multichannel=True)  # comparing the image for finding difference using compare_ssim function
    return score
